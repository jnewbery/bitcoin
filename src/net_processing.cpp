...
struct Peer {
    ...
    bool fPreferredDownload GUARDED_BY(m_mutex_message_handling) {false};

    explicit Peer(NodeId id, Mutex& mutex_message_handling)
        : m_id(id)
...
private:
    ...
    void _ProcessMessage(CNode& pfrom, const std::string& msg_type, CDataStream& vRecv,
                        const std::chrono::microseconds time_received, const std::atomic<bool>& interruptMsgProc) EXCLUSIVE_LOCKS_REQUIRED(m_mutex_message_handling);
    ...
    int nPreferredDownload GUARDED_BY(m_mutex_message_handling) = 0;
};
...
void PeerManagerImpl::_RelayTransaction(const uint256& txid, const uint256& wtxid)
{
    ...
    m_connman.ForEachNode([this, &txid, &wtxid](CNode* node) EXCLUSIVE_LOCKS_REQUIRED(m_mutex_message_handling, m_peer_mutex) {
        AssertLockHeld(m_peer_mutex);
        AssertLockHeld(m_mutex_message_handling);
        auto it = m_peer_map.find(node->GetId());
        if (it == m_peer_map.end()) return false;
