...
class TestNode(NodeConnCB):
    ...
    def check_last_announcement(self, headers=None, inv=None):
        ...
        with mininode_lock:
            ...
            success = True
            compare_inv = []
            if self.last_message.get("inv"):
                compare_inv = [x.hash for x in self.last_message["inv"].inv]
            if compare_inv != expect_inv:
                success = False

            hash_headers = []
            if self.last_message.get("headers"):
    ...
    def wait_for_getdata(self, hash_list, timeout=60):
        ...
        test_function = lambda: self.last_message.get("getdata") and [x.hash for x in self.last_message["getdata"].inv] == hash_list
    ...
    def run_test(self):
        ...
        for i in range(10):
            ...
            for j in range(2):
                ...
                # broadcast it)
                assert not inv_node.last_message.get("inv")
                assert not inv_node.last_message.get("headers")
                tip = self.mine_blocks(1)
                assert_equal(inv_node.check_last_announcement(inv=[tip]), True)
                assert_equal(test_node.check_last_announcement(headers=[tip]), True)
                height += 1
        ...
        with mininode_lock:
            assert not test_node.last_message.get("getdata")
        ...
        for i in range(10):
            ...
            with mininode_lock:
                test_node.last_message["getheaders"] = None
        ...
        # throughout the test
        assert not inv_node.last_message.get("getdata")
