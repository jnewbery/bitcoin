...
void PeerManager::FinalizeNode(const CNode& node, bool& fUpdateConnectionTime) {
    ...
    if (node.m_connection_state == ConnectionState::FULLY_CONNECTED &&
...
template<>
void PeerManager::ProcessMessageType<MSG_TYPE::VERSION>(CNode& pfrom, Peer& peer,
...
template<>
void PeerManager::ProcessMessageType<MSG_TYPE::SENDCMPCT>(CNode& pfrom, Peer& peer,
                                                          const std::string& msg_type,
                                                          CDataStream& vRecv,
                                                          const std::chrono::microseconds time_received,
                                                          const std::atomic<bool>& interruptMsgProc)
...
template<>
void PeerManager::ProcessMessageType<MSG_TYPE::TX>(CNode& pfrom, Peer& peer,
                                                   const std::string& msg_type,
                                                   CDataStream& vRecv,
                                                   const std::chrono::microseconds time_received,
                                                   const std::atomic<bool>& interruptMsgProc)
...
template<>
void PeerManager::ProcessMessageType<MSG_TYPE::CMPCTBLOCK>(CNode& pfrom, Peer& peer,
                                                           const std::string& msg_type,
                                                           CDataStream& vRecv,
                                                           const std::chrono::microseconds time_received,
                                                           const std::atomic<bool>& interruptMsgProc)
...
template<>
void PeerManager::ProcessMessageType<MSG_TYPE::PONG>(CNode& pfrom, Peer& peer,
                                                     const std::string& msg_type,
                                                     CDataStream& vRecv,
                                                     const std::chrono::microseconds time_received,
                                                     const std::atomic<bool>& interruptMsgProc)
...
template<>
void PeerManager::ProcessMessageType<MSG_TYPE::GETCFILTERS>(CNode& pfrom, Peer& peer,
                                                            const std::string& msg_type,
                                                            CDataStream& vRecv,
                                                            const std::chrono::microseconds time_received,
                                                            const std::atomic<bool>& interruptMsgProc)
...
template<>
void PeerManager::ProcessMessageType<MSG_TYPE::GETCFCHECKPT>(CNode& pfrom, Peer& peer,
                                                             const std::string& msg_type,
                                                             CDataStream& vRecv,
                                                             const std::chrono::microseconds time_received,
                                                             const std::atomic<bool>& interruptMsgProc)
{
    ...
    const CBlockIndex* stop_index;
    BlockFilterIndex* filter_index;
    if (!PrepareBlockFilterRequest(pfrom, m_chainparams, filter_type, /*start_height=*/0, stop_hash,
...
bool PeerManager::SendMessages(CNode* pto)
{
    ...
    if (pto->m_connection_state != ConnectionState::FULLY_CONNECTED || pto->fDisconnect) {
        return true;
    }
