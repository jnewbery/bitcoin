...
class IncludeConfTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.restart_node(0, extra_args=["-includeconf=relative.conf"])

        subversion = self.nodes[0].getnetworkinfo()["subversion"]
        assert subversion.endswith("main; relative)/")

        self.log.info("-includeconf cannot be used recursively. subversion should with 'main)/'")
        with open(os.path.join(self.options.tmpdir + "/node0", "relative.conf"), "w", encoding="utf8") as f:
        ...
        subversion = self.nodes[0].getnetworkinfo()["subversion"]
        assert subversion.endswith("main)/")

        self.log.info("multiple -includeconf args can be used from the base config file. subversion should with 'main)/'")
        with open(os.path.join(self.options.tmpdir + "/node0", "relative.conf"), "w", encoding="utf8") as f:
            f.write("uacomment=relative\n")
        with open(os.path.join(self.options.tmpdir + "/node0", "bitcoin.conf"), 'a', encoding='utf8') as f:
