...
bool AppInitMain(NodeContext& node)
{
    ...
    if (script_threads <= 0) {
        ...
        // (-1 because the main thread counts towards the par threads)
        script_threads += GetNumCores() - 1;
    }
    // 0 <= number of script-checking threads <= MAX_SCRIPTCHECK_THREADS
    script_threads = std::min(std::max(script_threads, 0), MAX_SCRIPTCHECK_THREADS);
