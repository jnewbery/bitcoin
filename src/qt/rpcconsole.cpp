...
#ifdef ENABLE_WALLET
                            LogPrint(BCLog::QT, "trying to add wallet to request uri");
                            CWalletRef * const ppwallet = (CWalletRef*)ppwallet_v;
                            if (ppwallet) {
                                req.URI = "/wallet/" + (*ppwallet)->GetName();
                                LogPrint(BCLog::QT, "endpoint = %s", req.URI);
                                delete ppwallet;
                            }
#endif
                            LogPrint(BCLog::QT, "sending request");
...
void RPCExecutor::request(const QString &command, void *ppwallet_v)
{
    try
    {
        std::string result;
