...
{
    auto txinfo = mempool.info(txid_or_wtxid.GetHash(), txid_or_wtxid.IsWtxid());
    if (txinfo.tx) {
...

    {
        ...
        if (State(peer.GetId())->m_recently_announced_invs.contains(txid_or_wtxid.GetHash())) {
            // If it was, it can be relayed from either the mempool...
            if (txinfo.tx) return std::move(txinfo.tx);
            // ... or the relay pool.
            auto mi = mapRelay.find(txid_or_wtxid.GetHash());
