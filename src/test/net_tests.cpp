...
BOOST_AUTO_TEST_CASE(cnode_simple_test)
{
    ...
    std::unique_ptr<CNode> pnode1 = MakeUnique<CNode>(id++, height, hSocket, addr, 0, 0, CAddress(), pszDest, fInboundIn);
    BOOST_CHECK(pnode1->fInbound == false);
    BOOST_CHECK(pnode1->fFeeler == false);

    fInboundIn = true;
    std::unique_ptr<CNode> pnode2 = MakeUnique<CNode>(id++, height, hSocket, addr, 1, 1, CAddress(), pszDest, fInboundIn);
...
BOOST_AUTO_TEST_CASE(ipv4_peer_with_ipv6_addrMe_test)
{
    ...
    std::unique_ptr<CNode> pnode = MakeUnique<CNode>(0, 0, INVALID_SOCKET, addr, 0, 0, CAddress{}, std::string{}, false);
    ...
    GetPeerLocalAddr(&*pnode, NODE_NETWORK);
