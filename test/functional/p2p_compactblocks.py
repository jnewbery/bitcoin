...
class CompactBlocksTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.segwit_peer_1 = self.nodes[0].add_p2p_connection(TestP2PConn(cmpct_version=2))
        self.segwit_peer_2 = self.nodes[0].add_p2p_connection(TestP2PConn(cmpct_version=2))
        self.legacy_peer = self.nodes[0].add_p2p_connection(TestP2PConn(cmpct_version=1), services=NODE_NETWORK)
        ...
        self.request_cb_announcements(self.legacy_peer)
        self.request_cb_announcements(self.segwit_peer_1)
    ...
    def make_utxos(self):
        ...
        self.segwit_peer_1.send_and_ping(msg_block(block))
        ...
        block2.solve()
        self.segwit_peer_1.send_and_ping(msg_block(block2))
    ...
    # If legacy_peer is passed in, request compact blocks with version=preferred-1
    # and verify that it receives block announcements via compact block.
    def test_sendcmpct(self, peer, legacy_peer=None):
        preferred_version = peer.cmpct_version
        node = self.nodes[0]
        ...
        with mininode_lock:
            ...
            assert_equal(peer.last_sendcmpct[0].version, preferred_version)
        ...
        check_announcement_of_new_block(node, peer, lambda p: "cmpctblock" not in p.last_message)

        # Try one more time, this time after requesting headers.
        peer.request_headers_and_sync(locator=[tip])
        check_announcement_of_new_block(node, peer, lambda p: "cmpctblock" not in p.last_message and "inv" in p.last_message)
        ...
        sendcmpct.announce = True
        peer.send_and_ping(sendcmpct)
        check_announcement_of_new_block(node, peer, lambda p: "cmpctblock" in p.last_message)
        ...
        sendcmpct.announce = False
        peer.send_and_ping(sendcmpct)
        check_announcement_of_new_block(node, peer, lambda p: "cmpctblock" not in p.last_message and "headers" in p.last_message)
        ...
        if legacy_peer is not None:
            ...
            check_announcement_of_new_block(node, legacy_peer, lambda p: "cmpctblock" in p.last_message)

    # Compare the generated shortids to what we expect based on BIP 152, given
    # bitcoind's choice of nonce.
    def test_compactblock_construction(self, peer, use_witness_address=True):
        version = peer.cmpct_version
        node = self.nodes[0]
    ...
    def test_compactblock_requests(self, peer, segwit=True):
        version = peer.cmpct_version
        node = self.nodes[0]
    ...
    def test_getblocktxn_requests(self, peer):
        ...
        test_tip_after_message(node, peer, msg_bt, block.sha256)
        ...
        test_tip_after_message(node, peer, msg_cmpctblock(comp_block.to_p2p()), block.sha256)
        with mininode_lock:
    ...
    def test_invalid_cmpctblock_message(self):
        ...
        self.segwit_peer_1.send_await_disconnect(msg_cmpctblock(cmpct_block))
