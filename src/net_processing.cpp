...
struct Peer {
    ...
    std::atomic<int64_t> m_ping_sent_time{0};
    /** Last measured ping round-trip time */
    std::atomic<int64_t> m_ping_roundtrip_time{0};
...
bool IsAddrRelayPeer(Peer& peer) { return peer.m_addr_known != nullptr; }
...
void PushAddress(Peer& peer, const CAddress& addr, FastRandomContext &insecure_rand)
{
...
void AddInventoryKnown(PeerRef& peer, const CInv& inv)
{
...
static void UpdatePreferredDownload(const CNode& node, Peer& peer)
{
    ...
    peer.m_preferred_download = (!node.fInbound || node.HasPermission(PF_NOBAN)) && !node.fOneShot && CanServeBlocks(peer);
...
static void PushNodeVersion(CNode& pnode, CConnman* connman, int64_t nTime)
{
    ...
    connman->PushMessage(&pnode, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::VERSION, PROTOCOL_VERSION, (uint64_t)nLocalNodeServices, nTime, addrYou, addrMe,
            nonce, strSubVersion, nNodeStartingHeight, ::g_relay_txes && peer->m_tx_relay != nullptr));
...
bool GetNodeStateStats(NodeId nodeid, CNodeStateStats &stats) {
    ...
    }
    {
        ...
        if ((0 != peer->m_ping_nonce_sent) && (0 != peer->m_ping_sent_time)) {
            nPingUsecWait = GetTimeMicros() - peer->m_ping_sent_time;
        }

        // Raw ping time is in microseconds, but show it to user as whole seconds (Bitcoin users should be well used to small numbers with many decimal places by now :)
        stats.m_ping_usec = peer->m_ping_roundtrip_time;
        ...
        if (peer->m_tx_relay != nullptr) {
            ...
            stats.m_fee_filter_theirs = WITH_LOCK(peer->m_tx_relay->m_fee_filter_mutex, return peer->m_tx_relay->m_fee_filter_theirs);
        } else {
...
void PeerLogicValidation::UpdatedBlockTip(const CBlockIndex *pindexNew, const CBlockIndex *pindexFork, bool fInitialDownload) {
    ...
    if (!fInitialDownload) {
        ...
        ForEachPeer([nNewHeight, &vHashes](const PeerRef& peer) {
            ...
            if (nNewHeight > (peer->m_starting_height != -1 ? peer->m_starting_height - 2000 : 0)) {
...
static void RelayAddress(const CAddress& addr, bool fReachable, const CConnman& connman)
{
    ...
    std::array<std::pair<uint64_t, PeerRef>,2> best{{{0, nullptr}, {0, nullptr}}};
    assert(nRelayNodes <= best.size());

    LOCK(g_peer_mutex);

    for (auto peer_it : g_peer_map) {
        PeerRef peer = peer_it.second;
        if (IsAddrRelayPeer(*peer)) {
            uint64_t hashKey = CSipHasher(hasher).Write(peer->m_id).Finalize();
            for (unsigned int i = 0; i < nRelayNodes; i++) {
                 if (hashKey > best[i].first) {
                     std::copy(best.begin() + i, best.begin() + nRelayNodes - 1, best.begin() + i + 1);
                     best[i] = std::make_pair(hashKey, peer);
                     break;
                 }
            }
        }
...
CTransactionRef static FindTxForGetData(PeerRef& peer, const uint256& txid, const std::chrono::seconds mempool_req, const std::chrono::seconds longlived_mempool_time) LOCKS_EXCLUDED(cs_main)
{
    ...
    {
        LOCK(peer->m_tx_relay->m_tx_inventory_mutex);
        if (peer->m_tx_relay->m_tx_inventory_to_send.count(txid)) return {};
    }
...
void static ProcessGetData(CNode& pfrom, const CChainParams& chainparams, CConnman* connman, CTxMemPool& mempool, const std::atomic<bool>& interruptMsgProc) LOCKS_EXCLUDED(cs_main)
{
    ...
    while (it != peer->m_inventory_requested.end() && (it->type == MSG_TX || it->type == MSG_WITNESS_TX)) {
        ...
        CTransactionRef tx = FindTxForGetData(peer, inv.hash, mempool_req, longlived_mempool_time);
        if (tx) {
...
static void ProcessHeadersMessage(CNode& pfrom, CConnman* connman, ChainstateManager& chainman, CTxMemPool& mempool, const std::vector<CBlockHeader>& headers, const CChainParams& chainparams, bool via_compact_block)
{
    ...
    const CBlockIndex *pindexLast = nullptr;
    {
        ...
        if (!LookupBlockIndex(headers[0].hashPrevBlock) && nCount < MAX_BLOCKS_TO_ANNOUNCE) {
            ...
            if (nodestate->nUnconnectingHeaders % MAX_UNCONNECTING_HEADERS == 0) {
                Misbehaving(pfrom.GetId(), 20, strprintf("%d unconnecting headers", nodestate->nUnconnectingHeaders));
            }
            return;
        }
...

    {
        ...
        if (fCanDirectFetch && pindexLast->IsValid(BLOCK_VALID_TREE) && ::ChainActive().Tip()->nChainWork <= pindexLast->nChainWork) {
            ...
            while (pindexWalk && !::ChainActive().Contains(pindexWalk) && vToFetch.size() <= MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
                if (!(pindexWalk->nStatus & BLOCK_HAVE_DATA) &&
                        ...
                        (!IsWitnessEnabled(pindexWalk->pprev, chainparams.GetConsensus()) || CanServeWitnesses(*peer))) {
        ...
        if (!pfrom.fDisconnect && IsOutboundDisconnectionCandidate(pfrom) && nodestate->pindexBestKnownBlock != nullptr && peer->m_tx_relay != nullptr) {
    ...
    const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::VERACK)
    {
        ...
        if (!pfrom.fInbound) {
            ...
            LogPrintf("New outbound peer connected: version: %d, blocks=%d, peer=%d%s (%s)\n",
                      ...
                      peer->m_tx_relay == nullptr ? "block-relay" : "full-relay");
        }
    ...
    if (msg_type == NetMsgType::INV) {
        ...
        bool fBlocksOnly = !g_relay_txes || (peer->m_tx_relay == nullptr);
    ...
    if (msg_type == NetMsgType::TX) {
        ...
        if ((!g_relay_txes && !pfrom.HasPermission(PF_RELAY)) || (peer->m_tx_relay == nullptr))
        {
        ...
        if (nAvail >= sizeof(nonce)) {
            ...
            if (peer->m_ping_nonce_sent != 0) {
                if (nonce == peer->m_ping_nonce_sent) {
                    ...
                    bPingFinished = true;
                    int64_t pingUsecTime = pingUsecEnd - peer->m_ping_sent_time;
                    if (pingUsecTime > 0) {
                        // Successful ping time measurement, replace previous
                        peer->m_ping_roundtrip_time = pingUsecTime;
    ...
    if (msg_type == NetMsgType::FEEFILTER) {
        ...
        if (MoneyRange(newFeeFilter)) {
            if (peer->m_tx_relay != nullptr) {
                LOCK(peer->m_tx_relay->m_fee_filter_mutex);
...
bool PeerLogicValidation::MaybeDiscourageAndDisconnect(CNode& pnode)
{
    ...
    // Normal case: Disconnect and discourage all nodes sharing the address
...
bool MaybeSendPing(CNode& pto, CConnman& connman)
{
    ...
    const CNetMsgMaker msgMaker(pto.GetSendVersion());
    bool pingSend = false;
    if (peer->m_ping_sent_time + PING_INTERVAL * 1000000 < GetTimeMicros()) {
        if (peer->m_ping_nonce_sent) {
            // Ping has timed out. Disconnect peer.
            LogPrint(BCLog::NET, "ping timeout: %fs, peer=%s\n", 0.000001 * (GetTimeMicros() - peer->m_ping_sent_time), pto.GetId());
            pto.fDisconnect = true;
            return true;
        } else {
    ...
    if (pingSend) {
        ...
        peer->m_ping_sent_time = GetTimeMicros();
...
void MaybeSendAddr(CNode& pto, CConnman& connman)
{
    ...
    if (!IsAddrRelayPeer(*peer)) return;
    ...
    if (peer->m_next_addr_send >= current_time) {
...
bool PeerLogicValidation::SendMessages(CNode* pto)
{
...

    {
        ...
        bool fFetch = peer->m_preferred_download || (nPreferredDownload == 0 && CanServeBlocks(*peer) && !pto->fOneShot); // Download if this is a nice peer, or we have no nice peers and this one might do.
        ...
        std::vector<CInv> vInv;
        {
            ...
            if (peer->m_tx_relay != nullptr) {
                ...
                if (peer->m_tx_relay->m_next_inv_send_time < current_time) {
                    fSendTrickle = true;
                    if (pto->fInbound) {
                        peer->m_tx_relay->m_next_inv_send_time = std::chrono::microseconds{connman->PoissonNextSendInbound(nNow, INVENTORY_BROADCAST_INTERVAL)};
                    } else {
                        // Use half the delay for outbound peers, as there is less privacy concern for them.
                        peer->m_tx_relay->m_next_inv_send_time = PoissonNextSend(current_time, std::chrono::seconds{INVENTORY_BROADCAST_INTERVAL >> 1});
                    }
                }
                ...
                if (fSendTrickle && peer->m_tx_relay->m_send_mempool) {
                    ...
                    {
                        LOCK(peer->m_tx_relay->m_fee_filter_mutex);
                        filterrate = CFeeRate(peer->m_tx_relay->m_fee_filter_theirs);
                    }
                    ...
                    peer->m_tx_relay->m_last_mempool_req = GetTime<std::chrono::seconds>();
                }
        ...
        // Message: feefilter
        //
        if (peer->m_tx_relay != nullptr && pto->nVersion >= FEEFILTER_VERSION && gArgs.GetBoolArg("-feefilter", DEFAULT_FEEFILTER) &&
            ...
            if (m_chainman.ActiveChainstate().IsInitialBlockDownload()) {
                ...
                if (peer->m_tx_relay->m_fee_filter_ours == MAX_FILTER) {
                    ...
                    peer->m_tx_relay->m_fee_filter_next_send = timeNow - 1;
                }
            }
            if (timeNow > peer->m_tx_relay->m_fee_filter_next_send) {
                ...
                peer->m_tx_relay->m_fee_filter_next_send = PoissonNextSend(timeNow, AVG_FEEFILTER_BROADCAST_INTERVAL);
            }
            // If the fee filter has changed substantially and it's still more than MAX_FEEFILTER_CHANGE_DELAY
            // until scheduled broadcast, then move the broadcast to within MAX_FEEFILTER_CHANGE_DELAY.
            else if (timeNow + MAX_FEEFILTER_CHANGE_DELAY * 1000000 < peer->m_tx_relay->m_fee_filter_next_send &&
                     (currentFilter < 3 * peer->m_tx_relay->m_fee_filter_ours / 4 || currentFilter > 4 * peer->m_tx_relay->m_fee_filter_ours / 3)) {
                peer->m_tx_relay->m_fee_filter_next_send = timeNow + GetRandInt(MAX_FEEFILTER_CHANGE_DELAY) * 1000000;
            }
        }
    } // release cs_main
    return true;
