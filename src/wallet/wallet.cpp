...
CAmount CWallet::GetBalance(const isminefilter& filter, const int min_depth, const bool include_untrusted) const
{
    CAmount nTotal = 0;
    {
        auto locked_chain = chain().lock();
        ...
        for (const auto& entry : mapWallet)
        {
            ...
            bool use_cache = min_depth == 0;
            nTotal += pcoin->GetAvailableCredit(*locked_chain, use_cache, filter, min_depth);
        }
    }
